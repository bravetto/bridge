...
function registerCommands(context) {
    ...
    const openDashboard = vscode.commands.registerCommand('biasguard.openDashboard', () => {
        ...
        const dashboardUrl = config.get('dashboardUrl', 'http://localhost:1437/bias-dashboard');
    ...
    const analyzeConversation = vscode.commands.registerCommand('biasguard.analyzeConversation', async () => {
        ...
        if (result.score >= 30) {
            ...
            vscode.window.showWarningMessage(`BiasGuard: ${result.level} (${result.score}%) - Check analysis document`, 'View Details').then(selection => {
 ...
 */
function enhancedBiasCheck(text) {
    let score = 0;
    const problems = [];
 ...
 */
function startBiasMonitoring(context) {
    const config = vscode.workspace.getConfiguration('biasguard');
    ...
    const documentChangeListener = vscode.workspace.onDidChangeTextDocument(async (event) => {
        ...
        if (change.text.length > 50) { // Significant text addition
            ...
            if (result.score >= 60) { // High bias threshold for auto-detection
                ...
                vscode.window.showWarningMessage(`BiasGuard: ${result.level} detected in recent changes`, 'Analyze').then(selection => {
 ...
 */
function showBiasOverlay(context) {
    webviewPanel = vscode.window.createWebviewPanel('biasguardOverlay', 'BiasGuard Monitor', vscode.ViewColumn.Beside, {
        enableScripts: true,
...
function getBiasOverlayHtml() {
    ...
    <body>
        ...
        <div class="buttons">
            <button onclick="analyzeCurrentText()">Analyze Current Text</button>
