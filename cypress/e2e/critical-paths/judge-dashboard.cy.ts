describe('Judge Dashboard Access', () => {
  ...
  it('provides secure access to judge-specific content', () => {
    ...
    cy.get('body').should(($body) => {
      ...
      const hasSummarySection = summarySelectors.some((selector) => $body.find(selector).length > 0)
    ...
    cy.get('body').should(($body) => {
      ...
      const hasMetricsSection = metricsSelectors.some((selector) => $body.find(selector).length > 0)
    ...
    cy.get('body').should(($body) => {
      ...
      const hasSupportSection = supportSelectors.some((selector) => $body.find(selector).length > 0)
    ...
    cy.get('body').then(($body) => {
      ...
      const hasVisualization = chartSelectors.some((selector) => $body.find(selector).length > 0)
  ...
  it('displays comprehensive case information', () => {
    ...
    const expectedContent = ['jahmere', 'webb', 'case', 'justice', 'transformation', 'support']
    ...
    cy.get('body').should(($body) => {
      ...
      const caseKeywords = ['jahmere', 'webb', 'case', 'justice', 'transformation']
      ...
      for (const selector of letterSelectors) {
        if ($body.find(selector).length > 0) {
          ...
          cy.get('body').should(($body) => {
            ...
            const hasPreviewContent = previewSelectors.some((sel) => $body.find(sel).length > 0)
  ...
  it('provides secure and professional presentation', () => {
    ...
    cy.get('body').should(($body) => {
      const devArtifacts = ['lorem ipsum', 'test', 'debug', 'placeholder', 'todo']

      const bodyText = $body.text().toLowerCase()
      const hasDevArtifacts = devArtifacts.some((artifact) => bodyText.includes(artifact))
    ...
    cy.get('a, button').each(($el) => {
      cy.wrap($el).should(($element) => {
        ...
        const hasAccessibleText =
          $element.text().trim().length > 0 || $element.attr('aria-label') || $element.attr('title')
  ...
  it('handles error states gracefully', () => {
    ...
    cy.get('body').should(($body) => {
      ...
      const bodyText = $body.text().toLowerCase()
      const hasRawErrors = errorMessages.some((error) => bodyText.includes(error))
  ...
  it('maintains consistent branding and messaging', () => {
    ...
    cy.get('body').should(($body) => {
      const brandingElements = ['bridge', 'jahmere', 'webb', 'transformation', 'justice']

      const bodyText = $body.text().toLowerCase()
      const brandingMatches = brandingElements.filter((element) => bodyText.includes(element))
    ...
    cy.get('body').should(($body) => {
      ...
      const bodyText = $body.text().toLowerCase()
      const hasUnprofessionalTerms = unprofessionalTerms.some((term) => bodyText.includes(term))
