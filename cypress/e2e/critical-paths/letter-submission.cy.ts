describe('Letter Submission Critical Path', () => {
  ...
  it('completes full letter submission flow from homepage to success', () => {
    ...
    cy.get('input[name*="name"], input[placeholder*="name"], #name, [data-testid*="name"]')
      .first()
    ...
    cy.wait(['@crmSubmit', '@letterSubmit'], { timeout: 10000 }).then((interceptions) => {
      // At least one should have been called
      expect(interceptions.some((i) => i.response?.statusCode === 200)).to.be.true
    })
  ...
  it('handles validation errors gracefully', () => {
    ...
    cy.get('button[type="submit"], button:contains("submit"), [data-testid*="submit"]')
      .first()
      .click()
    ...
    cy.get('body').should(($body) => {
      ...
      const hasEmailError = emailErrorIndicators.some((selector) => $body.find(selector).length > 0)
  ...
  it('preserves form data during navigation', () => {
    ...
    // Fill some data
    cy.get('input[name*="name"], [data-testid*="name"]').first().type('Jane Doe')
  ...
  it('handles network errors gracefully', () => {
    ...
    // Fill out form
    cy.get('input[name*="name"], [data-testid*="name"]').first().type('Network Test User')

    cy.get('input[type="email"], [data-testid*="email"]').first().type('test@example.com')
    ...
    cy.get('body').should(($body) => {
      ...
      const hasErrorFeedback = errorFeedback.some((selector) => $body.find(selector).length > 0)
