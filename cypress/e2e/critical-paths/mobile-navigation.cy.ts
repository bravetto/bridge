describe('Mobile Navigation', () => {
      ...
      for (const selector of mobileMenuSelectors) {
        if ($body.find(selector).length > 0) {
          ...
          cy.get('body').should(($body) => {
            ...
            const hasOpenMenu = openMenuSelectors.some((sel) => $body.find(sel).length > 0)
        ...
        if ($body.find(`a[href*="${page.url}"]`).length > 0) {
          ...
          for (const pattern of page.patterns) {
            if ($body.find(`:contains("${pattern}")`).length > 0) {
              cy.contains('a, button', new RegExp(pattern, 'i')).first().click()
              foundNavigation = true
              break
            }
          }
  ...
  it('validates touch target sizes meet accessibility standards', () => {
    ...
    cy.get('button, a, input, [role="button"], [tabindex="0"]').each(($element) => {
      cy.wrap($element).should(($el) => {
        ...
        const meetsEffectiveSize = effectiveWidth >= minSize && effectiveHeight >= minSize
  ...
  it('provides accessible mobile navigation', () => {
    ...
    cy.focused().should(($el) => {
      ...
      expect(hasOutline || hasBoxShadow || hasBorder || hasBackground).to.be.true
    })
    ...
    cy.get('body').then(($body) => {
      if ($body.find('a[href="#main"], a[href="#content"]').length > 0) {
        cy.get('a[href="#main"], a[href="#content"]').should('exist').and('contain.text', /skip/i)
      }
    })
  })
  ...
  it('displays content properly on various mobile sizes', () => {
    ...
    mobileViewports.forEach((viewport) => {
      ...
      cy.get('body').should(($body) => {
        ...
        expect(hasHorizontalScroll, `${viewport.name} should not have horizontal scroll`).to.be
          .false
      })
      ...
      cy.get('body')
        ...
        .then((fontSize) => {
          ...
          expect(fontSizeValue, 'Font size should be at least 16px on mobile').to.be.at.least(16)
        })
      ...
      cy.get('button, a').each(($el) => {
        cy.wrap($el).should(($element) => {
          ...
          expect(rect.height, 'Interactive elements should be at least 32px tall').to.be.at.least(
            32,
          )
        })
      })
