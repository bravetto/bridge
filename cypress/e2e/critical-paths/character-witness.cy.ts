describe('Character Witness Display', () => {
    ...
    it(`displays ${witness} information correctly`, () => {
      ...
      cy.get('body').then(($body) => {
        ...
        cy.get('body').should(($body) => {
          ...
          const hasNameElement = nameSelectors.some((selector) => $body.find(selector).length > 0)
        ...
        cy.get('body').should(($body) => {
          ...
          const hasRoleElement = roleSelectors.some((selector) => $body.find(selector).length > 0)
        ...
        cy.get('body').should(($body) => {
          ...
          const hasBioElement = bioSelectors.some(
            (selector) =>
              $body.find(selector).length > 0 && $body.find(selector).text().trim().length > 10,
          )
  ...
  it('navigates between witnesses correctly from people index', () => {
    ...
    cy.get('body').then(($body) => {
      ...
      if (!foundPersonLinks) {
        cy.log('No person links found on people index - testing direct navigation')
  ...
  it('handles missing person pages gracefully', () => {
    ...
    cy.get('body').should(($body) => {
      ...
      const hasErrorIndicator = errorIndicators.some((selector) => $body.find(selector).length > 0)
  ...
  it('displays responsive layout on mobile', () => {
    ...
    cy.get('body').then(($body) => {
      if ($body.find('[data-testid*="mobile"], .mobile-menu, .hamburger').length > 0) {
        cy.get('[data-testid*="mobile"], .mobile-menu, .hamburger').first().should('be.visible')
      }
    })
  })
      ...
      if ($images.length > 0) {
        $images.each((index, img) => {
          cy.wrap(img).should(($img) => {
            ...
            const hasFallback =
              $img.attr('src')?.includes('fallback') || $img.attr('src')?.includes('placeholder')
    ...
    cy.window().then((win) => {
      ...
      cy.get('body').should(($body) => {
        ...
        const hasVisibleError = boundarySelectors.some(
          (selector) =>
            $body.find(selector).is(':visible') && $body.find(selector).text().length > 0,
        )
    ...
    cy.window()
      ...
      .then((performance) => {
        ...
        if (navigation) {
          ...
          expect(navigation.domInteractive - navigation.fetchStart).to.be.lessThan(3000)
        }
      })
  })
})
