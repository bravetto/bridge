...
function validateValue(name, value, config) {
  ...
  if (value.includes('your_') || value.includes('_here') || value === 'your_value_here') {
    issues.push({
      level: 'error',
...
async function validateEnvironment() {
  ...
  if (!envLocal) {
    ...
    console.log(colorize('💡 Run "npm run env:setup" to generate template', 'yellow'))
    return false
  }
  ...
  for (const [name, config] of Object.entries(ENV_VARS)) {
    ...
    if (issues.length > 0) {
      ...
      issues.forEach((issue) => {
        ...
        const color =
          issue.level === 'error' ? 'red' : issue.level === 'warning' ? 'yellow' : 'blue'
  ...
  for (const name of Object.keys(envLocal)) {
    if (!ENV_VARS[name] && !name.startsWith('VERCEL_') && name !== 'NODE_ENV') {
      ...
      console.log(colorize('  ℹ️  Unknown variable (not in configuration)', 'blue'))
      console.log('')
    }
  }
  ...
  const totalRequired = Object.values(ENV_VARS).filter((v) => v.type === 'required').length
  const totalOptional = Object.values(ENV_VARS).filter((v) => v.type === 'optional').length
  const totalPublic = Object.values(ENV_VARS).filter((v) => v.type === 'public').length
  ...
  if (results.required.missing > 0) {
    console.log(colorize(`  ❌ ${results.required.missing} missing required variables`, 'red'))
  }
  if (results.required.invalid > 0) {
    console.log(colorize(`  ❌ ${results.required.invalid} invalid required variables`, 'red'))
  }

  console.log(
    colorize(`Optional Variables: ${results.optional.found}/${totalOptional} configured`, 'blue'),
  )
  console.log(
    colorize(`Public Variables: ${results.public.found}/${totalPublic} configured`, 'blue'),
  )

  if (results.unknown.found > 0) {
    console.log(colorize(`Unknown Variables: ${results.unknown.found} found`, 'yellow'))
  }
  ...
  if (deploymentReady) {
    ...
    console.log(colorize('All required variables are configured correctly.', 'green'))
  } else {
  ...
  if (!deploymentReady) {
    ...
    console.log('4. Test deployment: curl https://your-domain.vercel.app/api/health')
  }
...
async function checkVercelConfig() {
  ...
  try {
    ...
    if (vercelScripts.length > 0) {
      console.log(colorize(`✅ Found environment scripts: ${vercelScripts.join(', ')}`, 'green'))
    }
  } catch {
