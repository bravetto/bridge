...
class SafeCleanup {
  ...
  async executeCleanup() {
    ...
    try {
      ...
      console.log(`💾 Saved ${this.formatBytes(this.cleanupReport.sizeSaved)} of disk space`)
    } catch (error) {
  ...
  async removeSafeFiles() {
    ...
    const allSafeFiles = [...this.SAFE_FILES, ...this.SAFE_ARCHIVE_FILES, ...this.SAFE_SCRIPTS]
  ...
  async organizeFiles() {
    ...
    const scriptDirs = ['scripts/deployment', 'scripts/monitoring', 'scripts/maintenance']
...
`
  ...
  async safeDelete(filePath) {
    try {
      ...
      this.cleanupReport.errors.push(`Failed to delete ${filePath}: ${error.message}`)
    }
  }
