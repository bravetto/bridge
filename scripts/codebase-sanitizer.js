...
class CodebaseSanitizer {
  ...
  async postCleanupValidation() {
    ...
    if (failedTests.length > 0) {
      throw new Error(`Validation failed: ${failedTests.map((t) => t.test).join(', ')}`)
    }
...
`
  ...
  async organizeScripts() {
    ...
    const categories = {
      ...
      'scripts/maintenance/': ['clean-start.sh', 'daily-check.sh', 'check-progress.js'],
      ...
      try {
        ...
        for (const script of scripts) {
          ...
          if (await this.fileExists(sourcePath)) {
            ...
            this.cleanupReport.filesReorganized.push(`${sourcePath} → ${targetPath}`)
          }
        }
      } catch (error) {
