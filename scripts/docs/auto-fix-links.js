...
class LinkRepairer {
  constructor() {
    ...
    this.sectionMappings = {
      'COMPONENT_PATTERNS.md#button-patterns': 'docs/COMPONENT_LIBRARY.md#button-components',
    ...
    const scanDirectory = (dir) => {
      ...
      for (const entry of entries) {
        ...
        if (stat.isDirectory()) {
          ...
          if (
            !['node_modules', '.git', '.next', 'coverage', '.link-repair-backup'].includes(entry)
          ) {
  ...
  async findFixForBrokenLink(linkUrl, sourceFile) {
    ...
    if (this.fileMappings[fileName]) {
      return path.relative(sourceDir, path.join(this.rootDir, this.fileMappings[fileName]))
    }
