...
async function testAgentActivation() {
  ...
  try {
    ...
    if (
      healthContent.includes('getAgentOrchestrator') &&
      healthContent.includes("'agents') === 'true'")
    ) {
    ...
    log(
      '  3. Activate agents: curl "http://localhost:1437/api/monitoring/health?agents=true"',
      'blue',
    )
